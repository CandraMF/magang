            ...
            <div :class="{'overlay overlay-block rounded' : loading}">
                ...
                <div class="card-header">
                    <div class="card-title">
                        <router-link to="/admin/jurusan/form" >
                            <el-button :loading="loading" type="primary" icon="el-icon-plus text-white">Tambah Jurusan</el-button>
                        </router-link>
                    </div>
...
<script setup lang="ts">
    ...
    const getData = async (perPage: Number | null, page: Number | null) => {
        ...
        await axios.get('/sanctum/csrf-cookie').then(async (response) => {
            await axios.get('/api/major?perPage='+perPage+'&page='+page, {
                headers: {'Authorization': 'Bearer '+ token},
            })
            .then((response)=> {
                list.value = [];
