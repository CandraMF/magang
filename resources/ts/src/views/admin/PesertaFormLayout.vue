        ...
        <div class="card outline-dashed outline mt-5 card-custom mb-5 shadow-none">
            <div :class="{'overlay overlay-block rounded' : loading}">
                ...
                <div class="card-body pt-2 px-6 overlay-wrapper">
                    <div>
                        <PesertaForm ref="myForm" @finish-init="handleFinishInit" @success="handleSuccess" :data="formData"/>
                    </div>
                </div>
...
<script setup lang="ts">
    ...
    import PesertaForm from './forms/PesertaForm.vue';
    ...
    const modalTitle = ref('Tambah Peserta');
    let myModal = ref(null);

    let myForm = ref(null);

    onMounted(() => {
        setCurrentPageBreadcrumbs(modalTitle.value, ["Peserta", "Form"]);
    ...
    const handleSuccess = (payload) => {
        router.push({ name: 'admin-peserta', query: { redirect: '/admin/peserta' } });
    }
