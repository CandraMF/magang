<template>
    <div>
                ...
                <div class="card-body pt-2 px-6 overlay-wrapper">
                    <div>
                        <el-table
                            ...
                            <el-table-column sortable label="Tile" prop="family_type" />
                        ...
                        <el-button
                            @click="showModal()"
                            :loading="loading"
                            class="mt-4" style="width: 100%">Tambah Keluarga</el-button>
                    </div>
                </div>
        ...
        <Modal ref="myModal" size="lg" :title="modalTitle" @hide="handleHide">
            <template #body>
                <KeluargaForm ref="myForm" @success="handleSuccess" :data="formData"/>
            </template>
            <template #footer>
            </template>
        </Modal>
...
<script setup lang="ts">
    ...
    import KeluargaForm from '@/views/forms/KeluargaForm.vue';
    ...
    const modalTitle = ref('Tambah Keluarga');
    ...
    const getData = async () => {
        ...
        await axios.get('/sanctum/csrf-cookie').then(async (response) => {
            await axios.get('/api/family/'+personId.value, {
                headers: {'Authorization': 'Bearer '+ token},
            })
            .then((response)=> {
                list.value = [];
                ...
                response.data.family.forEach(element => {
