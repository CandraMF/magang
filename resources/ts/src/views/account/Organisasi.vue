<template>
    <div>
                ...
                <div class="card-body pt-2 px-6 overlay-wrapper">
                    <div>
                        <el-table
                            ...
                            <el-table-column sortable label="Organisasi" prop="organization" />
                        ...
                        <el-button
                            @click="showModal()"
                            :loading="loading"
                            class="mt-4" style="width: 100%">Tambah Riwayat Organisasi</el-button>
                    </div>
                </div>
        ...
        <Modal ref="myModal" size="lg" :title="modalTitle" @hide="handleHide">
            <template #body>
                <OrganisasiForm ref="myForm" @success="handleSuccess" :data="formData"/>
            </template>
            <template #footer>
            </template>
        </Modal>
...
<script setup lang="ts">
    ...
    import OrganisasiForm from '@/views/forms/OrganisasiForm.vue';
    ...
    const modalTitle = ref('Tambah Riwayat Organisasi');
    ...
    const getData = async () => {
        ...
        await axios.get('/sanctum/csrf-cookie').then(async (response) => {
            await axios.get('/api/organization/'+personId.value, {
                headers: {'Authorization': 'Bearer '+ token},
            })
            .then((response)=> {
                list.value = [];
                ...
                response.data.organization.forEach(element => {
